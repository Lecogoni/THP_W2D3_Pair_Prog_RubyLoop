...
def array_length
    array_length =  journalist.length
    puts "- Il y #{array_length} handle dans mon array"
end


def the_shorter
    puts "- le plus petit handle de la list est : "
...
def array_five
    puts "- il y a #{journalist.count {|i| i.length > 5 && i.length < 7 }} handle contenant 5 caractères"
end 
...
def short_to_big
    ...
    puts ""
    puts journalist.sort! { |a, b|  a.length <=> b.length }
end
